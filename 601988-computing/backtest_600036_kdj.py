# -*- coding: utf-8 -*-
"""
招商银行(600036) KDJ-J值 短线波段交易策略回测系统
目标：3万元全仓进出，每笔盈利1000左右（~3.3%），根据日线KDJ的J值买卖
高频操作，全年反复波段

基于中国银行(601988)同款KDJ策略，回测招商银行收益情况
"""
...
print("=" * 85)
print(f"  {STOCK_NAME}({STOCK_CODE}) KDJ-J值短线波段策略回测")
print(f"  资金: {CAPITAL:,}元 | 目标: 每笔±{TARGET_PROFIT}元 | T+1规则")
print(f"  策略来源: 中国银行(601988)同款KDJ策略")
print("=" * 85)

# ===================== 数据获取 =====================
print("\n[1/4] 获取数据...")

# 尝试通过yfinance获取（akshare在受限网络环境可能不可用）
CSV_PATH = "D:\\mycode\\601988-computing\\600036_yf.csv"
...
if not csv_exists:
    try:
        ...
        ticker = yf.Ticker(f"{STOCK_CODE}.SS")
        df_yf = ticker.history(start="2019-01-01", end="2026-06-08")
        df_yf.to_csv(CSV_PATH)
        df_raw = pd.DataFrame({
            "date": df_yf.index.tz_localize(None),
            "open": df_yf["Open"].values,
            "high": df_yf["High"].values,
            "low": df_yf["Low"].values,
            "close": df_yf["Close"].values,
            "volume": df_yf["Volume"].values,
        })
else:
    print(f"  从CSV加载: {CSV_PATH}")
    df_csv = pd.read_csv(CSV_PATH, index_col=0, parse_dates=True)
    df_raw = pd.DataFrame({
        "date": df_csv.index.tz_localize(None),
        "open": df_csv["Open"].values,
        "high": df_csv["High"].values,
        "low": df_csv["Low"].values,
        "close": df_csv["Close"].values,
        "volume": df_csv["Volume"].values,
    })
...
print(f"  当前价格: ¥{df_raw['close'].iloc[-1]:.2f}")
...
print("\n[4/4] 策略对比与推荐...\n")
print("=" * 95)
print(f"                    {STOCK_NAME}({STOCK_CODE}) KDJ-J值短线波段策略对比总览")
print("=" * 95)
...
# ===================== 与中国银行对比 =====================
print(f"\n--- 与中国银行(601988)策略适用性对比 ---")
print(f"  招商银行({STOCK_CODE})更适合KDJ策略的原因：")
print(f"  1. 股价更高(¥{df['close'].iloc[-1]:.2f})，波动幅度更大，短线空间更足")
print(f"  2. 日均成交量大，流动性好，滑点影响小")
print(f"  3. 银行股波动率适中，KDJ指标在震荡中信号质量较高")
print(f"  注意事项：")
print(f"  1. 招商银行波动大于中国银行，止损需适当放宽")
print(f"  2. 3万元资金可能只能买{int(CAPITAL/df['close'].iloc[-1]/100)*100}股（约{int(CAPITAL/df['close'].iloc[-1]/100)*100 * df['close'].iloc[-1]:,.0f}元仓位）")

# ===================== 可视化 =====================
print("\n生成图表...")

fig, axes = plt.subplots(2, 3, figsize=(20, 12))
fig.suptitle(f"{STOCK_NAME}({STOCK_CODE}) KDJ-J值短线波段策略回测\n"
...
plt.tight_layout()
chart_path = "D:\\mycode\\601988-computing\\backtest_600036_kdj_chart.png"
plt.savefig(chart_path, dpi=150, bbox_inches="tight")
print(f"  图表已保存: {chart_path}")

# ===================== 最终推荐 =====================
print("\n" + "=" * 85)
print(f"                        {STOCK_NAME}({STOCK_CODE}) 最终推荐")
print("=" * 85)
print(f"""
  [策略] {best_sum['name']}
...
# 保存报告
report_path = "D:\\mycode\\601988-computing\\backtest_600036_kdj_report.txt"
with open(report_path, "w", encoding="utf-8") as f:
    f.write("=" * 90 + "\n")
    f.write(f"{STOCK_NAME}({STOCK_CODE}) KDJ-J值短线波段策略回测报告\n")
    ...
    f.write(f"策略来源: 中国银行(601988)同款KDJ策略\n")
    f.write("=" * 90 + "\n\n")
